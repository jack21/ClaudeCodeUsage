...
이 확장 프로그램은 Claude Code의 대화 기록을 읽으며, Claude Code는
`cleanupPeriodDays` 설정(**기본값: 30일**)보다 오래된 기록을 자동으로 삭제합니다.
기록이 삭제되면 해당 사용량은 더 이상 표시할 수 없습니다.

더 많은 기록을 보관하려면 Claude Code 설정 파일(`~/.claude/settings.json`)에서
`cleanupPeriodDays` 값을 늘리세요:
...
이는 이후에 보관되는 데이터에만 적용되며, 이미 삭제된 기록은 복구할 수 없습니다.
