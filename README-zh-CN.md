...
此扩展会读取 Claude Code 的对话记录，而 Claude Code 会自动删除超过其
`cleanupPeriodDays` 设置（**默认:30 天**）的对话记录。一旦这些记录被删除，
对应的用量就无法再显示。

若要保留更多历史记录，请在 Claude Code 配置文件（`~/.claude/settings.json`）中
调高 `cleanupPeriodDays`:
