...
- **工作流标签页** —— 所有多代理运行尽收一处：动态工作流（ultracode）和临时子代理批次，含每次运行的成本、代理数、所用模型、**缓存命中率**（"我的供应商是否适配工作流"的诊断信号），以及按任务内容标注的逐代理明细（共同前缀置顶，仅展示差异部分）。
...
- **思考占比** —— 每会话的估算思考 token 占比（Sessions 列 + 今日卡片），过高时提示改用 `/effort high`。
- **工作流配额护栏** —— 当 5 小时窗口剩余不足以完成一次典型运行时，仪表盘显示可关闭的警告横幅（`claudeCodeUsage.workflowQuotaWarnPercent`，中断的运行会丢失提示缓存，重跑约贵 40%）。
- **AI 建议 2.0** —— 输入新增结构化信号（运行明细、各供应商缓存命中率、用量归因、思考占比）；可选 `advice.userContext` 设置让建议以"针对本项目的个性化建议"收尾。传输层加固：超时 + 重试 + curl 兜底，不再静默挂起。
