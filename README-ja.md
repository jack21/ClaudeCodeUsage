...
この拡張機能は Claude Code の会話トランスクリプトを読み取りますが、Claude Code
は `cleanupPeriodDays` 設定（**既定: 30 日**）より古いトランスクリプトを自動的に
削除します。ログが削除されると、その使用状況は表示できなくなります。

より多くの履歴を保持するには、Claude Code の設定ファイル
（`~/.claude/settings.json`）で `cleanupPeriodDays` を大きくしてください:
...
これは今後保持されるデータにのみ影響します。すでに削除されたトランスクリプトは
復元できません。
